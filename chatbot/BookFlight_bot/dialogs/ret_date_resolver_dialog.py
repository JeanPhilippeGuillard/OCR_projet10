...
class RetDateResolverDialog(CancelAndHelpDialog):
    def __init__(self, dialog_id: str = None):
        super(RetDateResolverDialog, self).__init__(
            dialog_id or RetDateResolverDialog.__name__
        )

        self.add_dialog(
            DateTimePrompt(
                DateTimePrompt.__name__, RetDateResolverDialog.datetime_prompt_validator
            )
        )
        self.add_dialog(
            WaterfallDialog(
    ...
    async def initial_step(
        ...
        if timex is None:
            prompt_msg_text = "On what date will you be flying back?"
